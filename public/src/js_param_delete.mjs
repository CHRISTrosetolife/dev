...
export async function js_param_delete(ast, function_name, param_name) {
  ...
  assert2(string_is, [param_name2]);
  assert2(string_is, [default_value_string2]);
  let needs_imports_add = false;
  js_param_generic(
    ast2,
    function_name2,
    function lambda(args) {
      let default_value = js_parse_expression(default_value_string2);
      lambda_add2(args, default_value);
      needs_imports_add = true;
    },
    (params) => {
      let param_new = js_parse_expression(param_name2);
      lambda_add2(params, param_new);
    },
  );
}
