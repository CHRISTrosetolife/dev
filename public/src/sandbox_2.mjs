...
export async function sandbox_2() {
  ...
  while (index_current < list_size(tokens)) {
    each_range_reverse(list_size(tokens) - index_current, (count) => {
      ...
      let padding = line_height_to_padding(height);
      if (width <= canvas_width) {
        list_add(lines, {
          height,
          text: sliced_text,
  ...
  each_index(lines, (line, index) => {
    ...
    let offset = list_map_sum(
      ...
      (line) => line.height + line_height_to_padding_double(line),
    );
  ...
  function line_height_to_padding_double(line) {
    return 2 * line_height_to_padding(line.height);
  }
