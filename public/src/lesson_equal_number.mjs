...
export function lesson_equal_number() {
  ...
  function screen_before1(parent) {
    html_p_text(
      parent,
      'if "a" and "b" are numbers and a and b are the same number , then we can write : a = b',
    );
    ...
    html_p_text(
      parent,
      "either two numbers are equal to each other , or they are not equal to each other",
    );
    html_p_text(
      parent,
      'if "a" and "b" are equal to each other then a = b is true',
    );
  }
  function screen_before2(parent) {
    html_p_text(
      parent,
      'if "a" and "b" are numbers , and "a" and "b" are different from each other then a = b is false',
    );
    html_p_text(parent, "for example :");
    let ul = html_ul(parent);
