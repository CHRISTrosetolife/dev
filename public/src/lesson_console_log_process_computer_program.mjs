...
export function lesson_console_log_process_computer_program(parent) {
    ...
    html_hr(parent);
    html_p_text(parent, 'the first computer programs we look at will output a message');
    html_p_text(parent, 'the first computer programs we look at will not have an input');
}
