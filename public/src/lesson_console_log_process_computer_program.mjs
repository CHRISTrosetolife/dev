import { html_p_text } from "./html_p_text.mjs";
export function lesson_console_log_process_computer_program(parent) {
  html_p_text(parent, "a computer program may have inputs");
  html_p_text(parent, "a computer program may have outputs");
  html_p_text(
    parent,
    "a computer program does something&nbsp;, using any inputs and producing any outputs",
  );
  html_p_text(
    parent,
    "we could say a computer program is a process that may have inputs and may have outputs",
  );
}
